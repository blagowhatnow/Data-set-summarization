...
def main(d_path, rep_percent, samp_percent,out_path):
    ...
    objFL = FacilityLocationMutualInformationFunction(n=embs.shape[0], data=np.array(embs), separate_rep=True, n_rep=len(samp), data_rep=np.array(samp), mode="dense", metric="cosine")
